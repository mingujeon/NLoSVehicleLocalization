...
class Particle:
    ...
    def random_initialize(self, _map):
        ...
        self.y = random.uniform(_map.road_range[2]+ 1, 8.5) 
    ...
    def move(self, t):
        ...
        self.v = self.v + random.normalvariate(0,1) # acceleration ~ N(0,1)
...
class ParticleFilter:
    ...
    def predict(self):
        ...
        # self.prediction.y = max(self.Map.road_range[2] ,min(self.Map.height, self.prediction.y))
    ...
    def render(self, save_dir, is_ray = False):
        ...
        # ego_vehicle
        ax.plot([0],[0],  'o', markersize=4, color='black')
        ...
        # prediction visualization -> Intensity avg
        ax.plot(self.prediction.x, 7.5, 'bo', markersize=7)
        ...
        if is_ray: # ploting ray
            ...
            for i,l in enumerate(reflect_2nd):
                x,y = l.xy
                ax.plot(x,y, c='red', alpha=0.6, label="2nd reflect" if i==0 else "") # green
            if len(direct) == 0:
                for i,l in enumerate(diffract):
                    x,y = l.xy
                    ax.plot(x,y, c='green', alpha=0.3, label="diffract" if i==0 else "") # blue
